...
__global__ void SeedA(const siphash_keys &sipkeys, ulonglong4 * __restrict__ buffer, u32 * __restrict__ indexes, const int seedCount) {
  ...
  for (int i = 0; i < loops; i+=EDGE_BLOCK_SIZE) {
      ...
      for(u32 e = 0; e < EDGE_BLOCK_SIZE; e++){
          ...
          if (counter == FLUSHA-1) {
              ...
              int cnt = atomicAdd(indexes + grp, nflush);//, (int)(maxOut - nflush));
              for (int i = 0; i < nflush && cnt <= maxOut-nflush; i += TMPPERLL4) {
...
__global__ void SeedB(const siphash_keys &sipkeys, const EdgeOut * __restrict__ source, ulonglong4 * __restrict__ destination, const u32 * __restrict__ sourceIndexes, u32 * __restrict__ destinationIndexes) {
  ...
  for (int loop = 0; loop < loops; loop++) {
    ...
    if (counter == FLUSHB-1) {
      ...
      int cnt = atomicAdd(destinationIndexes + grp, nflush);//, (int)(maxOut - nflush));
      for (int i = 0; i < nflush && cnt <= maxOut-nflush; i += TMPPERLL4)
...
struct trimparams {
  ...
  trimparams() {
    expand              = 0;//EDGEBITS >= 30 ? 3 : 0;
    ntrims              =  176;
    genA.blocks         = NXY;//4096;
    genA.tpb            =  128;
    genB.blocks         =  NXY;
    genB.tpb            =  128;
    trim.blocks         =  NXY;
...
struct edgetrimmer {
  ...
  u32 trim() {
    ...
    for(int i = 0; i < 1; i++){
    if (tp.expand == 0) {
      SeedA<EDGES_A, uint2><<<tp.genA.blocks, tp.genA.tpb>>>(*dipkeys, (ulonglong4*)bufferAB, (u32 *)indexesE[1], i);
    } else {
      SeedA<EDGES_A,   u32><<<tp.genA.blocks, tp.genA.tpb>>>(*dipkeys, (ulonglong4*)bufferAB, (u32 *)indexesE[1], i);
    }
    }
